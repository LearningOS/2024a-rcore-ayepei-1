...
impl TaskControlBlock {
    ...
    ///spawn:fork+exec
    pub fn spawn(self: &Arc<Self>,path: *const u8) -> Arc<Self>{
        let token = current_user_token();
        let path = translated_str(token, path);
        let (memory_set, user_sp, entry_point) = MemorySet::from_elf(open_file(path.as_str(), OpenFlags::RDONLY).unwrap().read_all().as_slice());
        let trap_cx_ppn = memory_set
            .translate(VirtAddr::from(TRAP_CONTEXT_BASE).into())
            .unwrap()
            .ppn();
