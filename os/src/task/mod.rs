...
impl TaskManager {
    ...
    ///mmap 匿名映射
    pub fn get_to_mmap(&self,start: usize, len: usize, port: usize)->isize{
        let mut inner = self.inner.exclusive_access();
        let cur = inner.current_task;
        inner.tasks[cur].change_mmap(start,len,port)
    }
    ///解除mmap 匿名映射
    pub fn get_to_munmap(&self,start: usize,len: usize)->isize{
        let mut inner = self.inner.exclusive_access();
        let cur = inner.current_task;
        inner.tasks[cur].change_munmmap(start,len) 
    }
    ...
    fn run_next_task(&self) {
        if let Some(next) = self.find_next_task() {
            ...
            if inner.run_time[next]==0{
                inner.run_time[next]=get_time_ms();
            }
    ...
    fn get_now(&self)-> usize{
        let inner = self.inner.exclusive_access();
        inner.run_time[inner.current_task]
    }
...
///获得系统调用次数
pub fn get_count()->[u32; MAX_SYSCALL_NUM]{
    TASK_MANAGER.get_count()
}


///创建mmap匿名映射
pub fn get_mmap(start: usize, len: usize, port: usize)->isize{
    TASK_MANAGER.get_to_mmap(start,len,port)
}

///解除mmap匿名映射
 pub fn get_munmap(start: usize,len: usize)->isize{
    TASK_MANAGER.get_to_munmap(start,len)
 }